...
class LSTM_LE:
    ...
    """
    def __init__(self, l2reg, dh, ne, nc, de, cs, emb):
        ...
        self.emb = ts('emb', emb.astype(th.config.floatX))
        ...
        self.emb_helper = ts('emb_helper', np.zeros((ne, de)).astype(th.config.floatX))
        ...
        self.params = [self.emb, self.h0, self.c0, self.wf, self.wi, self.wc, self.wo,\
                self.bf, self.bi, self.bc, self.bo, self.ws, self.bs]

        self.names = [p.name for p in self.params]

        self.params_helper = [self.emb_helper, self.h0_helper, self.c0_helper,\
...
if __name__ == "__main__":
    ...
    :params cs constext window size
    """
    model = LSTM(10, 2, 3, 4, 2)
    print(model.classify([[1, 0],[1, 0], [0,1]]))
